...
# Initialize lists to store accuracy, recall, ROC, and confusion matrices
accuracy_values = []
recall_values = []
roc_auc_values = []
confusion_matrices = []

# Define ARIMA order (you may need to tune this)
arima_order = (5, 1, 0)  # Example order; adjust as needed
...
for iteration in range(num_iterations):
    ...
    # Generate forecasts for training and testing sets
    y_train_forecasted = forecast_arima(y_train, arima_order)
    y_test_forecasted = forecast_arima(y_test, arima_order)

    # Classify based on the forecasted values
    threshold = 0.5
    y_train_predicted = (y_train_forecasted > threshold).astype(int)
    y_test_predicted = (y_test_forecasted > threshold).astype(int)

    # Calculate accuracy for this iteration and append to the list
    test_accuracy = accuracy_score(y_test, y_test_predicted)
    accuracy_values.append(test_accuracy)

    # Calculate recall for this iteration and append to the list
    test_recall = recall_score(y_test, y_test_predicted)
    recall_values.append(test_recall)

    # Calculate ROC AUC for this iteration and append to the list
    roc_auc = roc_auc_score(y_test, y_test_predicted)
    roc_auc_values.append(roc_auc)

    # Calculate confusion matrix for this iteration and append to the list
