...
for iteration in range(num_iterations):
    ...
    # Add a TCN layer with the specified number of units and kernel size
    model.add(TCN(input_shape=(x_train.shape[1], 1), nb_filters=64, kernel_size=2, nb_stacks=1, dilations=[1, 2, 4, 8]))
    ...
    # Reshape the input data to fit the TCN input shape
    x_train = x_train.reshape(x_train.shape[0], x_train.shape[1], 1)
    x_test = x_test.reshape(x_test.shape[0], x_test.shape[1], 1)
...
for i, accuracy in enumerate(accuracy_values):
    print(f"Iteration {i + 1}: Accuracy Score: {accuracy * 100:.2f}%")
